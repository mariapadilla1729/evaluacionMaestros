...
#librerias de modelos y regresion linea
...
def graficar(request):
    qs =cursos.objects.filter(aspectos='Total').order_by('puntaje') 
   #dataset = pd.read_csv('salarios.csv')
    qs =cursos.objects.filter(aspectos='Total').order_by('puntaje') 
    #dataX= [x.idDepartamento for x in qs]
    dataX=[p.idDepartamento for p in qs]   
    dataY=[dataY.puntaje for dataY in qs]
    dataset=[dataX,dataY]
    dataset.head(5) #mostrar los primeros 5 registros
    dataset.shape #muestro la dimension de los datos 30 filas; 2 columnas o variables
    #separar datos 20% prueba y 80% entrenamiento
    ...
    prediccion = regressor.predict(X_train) # es la linea de regresion lineal
    #Graficando los resultados de los datos de ENTRENAMIENTO
    plt.scatter(X_train, Y_train, color = 'blue')
    plt.plot(X_train, prediccion, color = 'black')
